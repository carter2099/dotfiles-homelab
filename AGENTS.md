...
- **Decision order:** private rule → secret/MAC/token scan (hold) → public rule, or a recorded decision for the same blob, or Jev for ambiguous paths → Jev must also clear every first publication and every change to a public file (credential/personal check) → hold when unsure (never publish on a guess).
...
**k3s is retired:** uninstalled on 2026-09-25 (no `k3s`/`kubectl`, no `~/k3s` or `~/.kube`; homelab-backup and the steward no longer depend on it). FreshRSS moved to Docker Compose. Details and rollback: [`k3s.md`](notes/docs/homelab/k3s.md).
...
- **Blog** (canonical `~/dev/blog/`; transactional deploy via `deploy/deploy.sh` — stop it with TERM, never KILL; production checkout `~/blog/blog/`; `714c218` (0.1.18) deployed 2026-09-25, check with `docker exec blog-web-1 cat config/version.rb`; the steward's P1 `app_deploy` deploys automatically when production is behind a CI-green `origin/main`; Docker loopback-only on 127.0.0.1:33099; public blog.carter2099.com tunnels directly to that origin, with no k3s dependency) → [`blog.md`](notes/docs/homelab/blog.md)
...
- [`k3s.md`](notes/docs/homelab/k3s.md) — k3s retirement record (uninstalled 2026-09-25)
- [`omp-web.md`](notes/docs/homelab/omp-web.md) — OMP Web retirement record (retired 2026-08-31)
